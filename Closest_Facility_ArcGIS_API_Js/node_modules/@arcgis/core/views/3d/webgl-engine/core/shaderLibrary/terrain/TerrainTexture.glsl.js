...
}`),t.fragment.code.add(e`vec4 getColor(vec4 color, vec2 uv, vec3 opacities) {
if (opacities.y <= 0.0) {
...
}`)):o.hasBackgroundColor?(t.fragment.uniforms.add("backgroundColor","vec3"),t.fragment.code.add(e`vec4 getColor(vec4 color, vec2 uv, vec3 opacities) {
if (opacities.y <= 0.0) {
return color * opacities.z * opacities.x;
}
float alpha = opacities.z * color.a;
...
return color;
}`),o.textureFadingEnabled?t.fragment.code.add(e`vec4 getTileColor() {
...
}`):t.fragment.code.add(e`vec4 getTileColor() {
