...
}`),a.fragment.code.add(t`vec3 correctForPlanetCurvature(vec3 dir)
{
dir.z = dir.z*(1.-radiusCurvatureCorrectionFactor) + radiusCurvatureCorrectionFactor;
return dir;
}`),a.fragment.code.add(t`vec3 rotateDirectionToAnchorPoint(mat4 rotMat, vec3 inVec)
{
return (rotMat * vec4(inVec, 0.0)).xyz;
}`),a.fragment.code.add(t`const float SUNSET_TRANSITION_FACTOR = 0.3;
...
}`),a.fragment.code.add(t`vec4 renderCloud(vec3 worldRay)
{
...
}`),a.fragment.code.add(t`vec4 renderCloudCrossFade(vec3 worldRay)
{
vec3 intersectionPoint = intersectWithCloudLayer(normalize(worldRay), cameraPosition, anchorPosition, 0.0);
vec3 worldRayRotated = rotateDirectionToAnchorPoint(rotationMatrixClouds, normalize(intersectionPoint));
vec3 worldRayRotatedCorrected = correctForPlanetCurvature(worldRayRotated);
...
}`)}export{a as CloudsParallaxShading};
